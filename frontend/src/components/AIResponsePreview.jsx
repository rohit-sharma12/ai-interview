      ...
      <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
        <ReactMarkdown
          ...
          components={{
            ...
            ul({ children }) {
              return <ul className="lis-disc pl-6 space-y-2 my-4">{children}</ul>
            },
            ol({ children }) {
            ...
            blockquote({ children }) {
              return <blockquote className="border-l-4 border-b-gray-200 pl-4 italic my-4">{children}</blockquote>
            },
            h1({ children }) {
            ...
            th({ children }) {
              return <th className='px-3 py-2 text-left font-medium uppercase text-gray-500 tracking-wider'>{children}</th>
            },
            td({ children }) {
              return <td className='px-3 py-3 whitespace-nowrap text-sm'>{children}</td>
            },
            hr() {
...
function CodeBlock({ code, language }) {
  ...
  return (
    <div className='relative m-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200'>
